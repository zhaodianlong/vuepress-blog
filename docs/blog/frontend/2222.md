...
再新建的 Tags.vue 文件中加入以下内容，这是我定义的 tag 组件，你们可以根据我的组件自行修改
...
Page 组件是文章显示的内容组件，如果需要在文章显示的时候把标签显示到文章头部，就需要改造 Page 组件。
